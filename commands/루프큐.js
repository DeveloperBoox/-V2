    ...
    run: async (client, message, args, { GuildDB }) => {
      ...
      if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":x: | **이 명령을 사용하려면 봇과 동일한 음성 채널에 있어야 합니다.!**");
        ...
        if (player.queueRepeat) {
          ...
          client.sendTime(message.channel, `:repeat: 대기열 루프 \'비활성화\``);
        } else {
          player.setQueueRepeat(true)
          client.sendTime(message.channel, `:repeat: 대기열 루프 \'활성화\``);
        }
    },
    SlashCommand: {
       /**
       ...
       */
        run: async (client, interaction, args, { GuildDB }) => {
          ...
          let awaitchannel = client.channels.cache.get(interaction.channel_id); /// thanks Reyansh for this idea ;-;
            ...
            if(player.queueRepeat){
                  player.setQueueRepeat(false)
                  client.sendTime(interaction, `:repeat: 대기열 루프 \'비활성화\``);
              }else{
                  player.setQueueRepeat(true)
                  client.sendTime(interaction, `:repeat: 대기열 루프 \'활성화\``);
              }
