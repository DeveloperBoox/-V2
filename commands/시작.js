...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!SearchString) return client.sendTime(message.channel, `**사용량 - **\`${GuildDB.prefix}시작 [song]\``);
        let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
        let Searching = await message.channel.send(":mag_right: 검색...");
        if (!CheckNode || !CheckNode.connected) {
       return client.sendTime(message.channel,"❌ | **라발링크 노드가 연결되지 않음**");
        }
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`재생 목록이 대기열에 추가됨`, message.author.displayAvatarURL());
                    ...
                    SongAddedEmbed.setAuthor(`대기열에 추가됨`, client.botconfig.IconURL);
                    SongAddedEmbed.setDescription(`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri})`);
                    SongAddedEmbed.addField("업로더", Searched.tracks[0].info.author, true);
                    //SongAddedEmbed.addField("Duration", `\`${prettyMilliseconds(Searched.tracks[0].length, { colonNotation: true })}\``, true);
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("대기열에 추가됨", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                } else {
                    return client.sendTime(message.channel, "**일치하는 항목이 없습니다. - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **지금은 아무것도 틀고있지 않아요...**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**일치하는 항목이 없습니다. - **" + SearchString);
                else if (Searched.loadType == "PLAYLIST_LOADED") {
                    player.queue.add(Searched.tracks);
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`재생 목록이 대기열에 추가됨`, client.botconfig.IconURL);
                    SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
                    SongAddedEmbed.setDescription(`[${Searched.playlist.name}](${SearchString})`);
                    SongAddedEmbed.addField("Enqueued", `\`${Searched.tracks.length}\` songs`, false);
                    SongAddedEmbed.addField("재생 목록 시간", `\`${prettyMilliseconds(Searched.playlist.duration, { colonNotation: true })}\``, false);
                    Searching.edit(SongAddedEmbed);
                } else {
                    ...
                    SongAddedEmbed.addField("업로더", Searched.tracks[0].author, true);
                    SongAddedEmbed.addField("시간", `\`${prettyMilliseconds(Searched.tracks[0].duration, { colonNotation: true })}\``, true);
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("대기열에 추가됨", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                }
            }
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    case "LOAD_FAILED":
                        ...
                        return client.sendError(interaction, `❌ | **검색하는 동안 오류가 발생했습니다.**`);

                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "❌ | **결과를 찾을 수 없습니다..**");
                    case "TRACK_LOADED":
                    ...
                    case "SEARCH_RESULT":
                        ...
                        let SongAdded = new MessageEmbed();
                            SongAdded.setAuthor(`대기열에 추가됨`, client.botconfig.IconURL);
                            SongAdded.setColor("RANDOM");
                            SongAdded.setDescription(`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri})`);
                            SongAdded.addField("업로더", Searched.tracks[0].info.author, true);
                            if (player.queue.totalSize > 1) SongAdded.addField("대기열에 위치", `${player.queue.size - 0}`, true);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        Playlist.setAuthor(`재생 목록이 대기열에 추가됨`, client.botconfig.IconURL);
                ...
                try {
                    ...
                    return client.sendError(interaction, `검색하는 동안 오류가 발생했습니다.: ${err.message}`);
                }
                switch (res.loadType) {
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                    ...
                    case "TRACK_LOADED":
                        ...
                        let SongAddedEmbed = new MessageEmbed();
                            ...
                            SongAddedEmbed.addField("업로더", res.tracks[0].author, true);
                            SongAddedEmbed.addField("시간", `\`${prettyMilliseconds(res.tracks[0].duration, { colonNotation: true })}\``, true);
                            if (player.queue.totalSize > 1) SongAddedEmbed.addField("추가됨", `${player.queue.size - 0}`, true);
                            return interaction.send(SongAddedEmbed);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        await player.play();
                        let SongAdded = new MessageEmbed();
                        SongAdded.setAuthor(`재생 목록이 대기열에 추가됨`, client.botconfig.IconURL);
                        ...
                        SongAdded.addField("재생목록 시간", `\`${prettyMilliseconds(res.playlist.duration, { colonNotation: true })}\``, false);
                        return interaction.send(SongAdded);
                    case "SEARCH_RESULT":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) {
                            ...
                            SongAddedEmbed.addField("시간", `\`${prettyMilliseconds(track.duration, { colonNotation: true })}\``, true);
                            if (player.queue.totalSize > 1) SongAddedEmbed.addField("추가됨", `${player.queue.size - 0}`, true);
                            player.play();
                            return interaction.send(SongAddedEmbed);
